...
scan_secrets() {
  ...
  if [ "${n:-0}" -gt 0 ]; then
    ...
    FINDINGS=$((FINDINGS + n))
    record gitleaks fail "${n} findings"
  else
    pass "no secrets detected"
    record gitleaks pass "0 findings"
  fi
}
...
scan_terraform() {
  ...
  local tfdirs
  tfdirs="$(terraform_dirs "$ROOT")"
  [ -n "$tfdirs" ] || {
    skip "no Terraform found"
    return 0
  }
  ...
  if have checkov; then
    ...
    local f="${report}/results_json.json"
    [ -r "$f" ] || f="$report"
    local n
    n="$(jq '[.. | .failed_checks? // empty | length] | add // 0' "$f" 2>/dev/null || echo 0)"
    if [ "${n:-0}" -gt 0 ]; then
      fail "${n} failed check(s)"
      jq -r '.. | .failed_checks? // empty | .[]? | "      \(.check_id) \(.file_path):\(.file_line_range[0])  \(.check_name)"' "$f" 2>/dev/null | head -20
      FINDINGS=$((FINDINGS + n))
      record checkov fail "${n} findings"
    else
      pass "checkov clean"
      record checkov pass "0 findings"
    fi
  else
    skip "checkov not installed"
    record checkov skipped ""
  fi
...
scan_deps() {
  ...
  local n
  n="$(jq '[.Results[]?.Vulnerabilities[]?] | length' "$report" 2>/dev/null || echo 0)"
  if [ "${n:-0}" -gt 0 ]; then
    ...
    FINDINGS=$((FINDINGS + n))
    record trivy-fs fail "${n} findings"
  else
    pass "no HIGH/CRITICAL dependency vulnerabilities"
    record trivy-fs pass "0 findings"
  fi
}
...
scan_code() {
  ...
  local n
  n="$(jq '.results | length' "$report" 2>/dev/null || echo 0)"
  if [ "${n:-0}" -gt 0 ]; then
    fail "${n} finding(s)"
    jq -r '.results[] | "      \(.path):\(.start.line)  \(.check_id)"' "$report" 2>/dev/null | head -20
    FINDINGS=$((FINDINGS + n))
    record semgrep fail "${n} findings"
  else
    pass "semgrep clean"
    record semgrep pass "0 findings"
  fi
...
scan_containers() {
  ...
  local files
  files="$(find "$ROOT" -maxdepth 3 \( -name 'Containerfile*' -o -name 'Dockerfile*' \) -not -path '*/.git/*' 2>/dev/null)"
  [ -n "$files" ] || {
    skip "no Containerfile/Dockerfile found"
    return 0
  }
  if have trivy; then
    local f
...
scan_actions() {
  ...
  if have actionlint; then
    # Capture first: `actionlint | head -30` SIGPIPEs actionlint on a repo
    # with many findings, and `pipefail` then reports a clean run as a failure.
    if al_out="$(cd "$ROOT" && actionlint -no-color 2>&1)"; then
      pass "actionlint clean"
    else
      printf '%s\n' "$al_out" | sed -n '1,30s/^/      /p'
      fail "actionlint findings"
    fi
  fi
  local unpinned
  ...
  unpinned="$(grep -rhoE 'uses:[[:space:]]*[A-Za-z0-9._-]+/[A-Za-z0-9._-]+@[A-Za-z0-9._/-]+' \
    "${ROOT}/.github/workflows" "${ROOT}/.github/actions" 2>/dev/null | grep -vE '@[0-9a-f]{40}$' | sort -u || true)"
  if [ -n "$unpinned" ]; then
